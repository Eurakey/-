...
// TODO：待补充代码
//页面主体部分
let j = 0, i = 0;//j限制每页课程数量，i用来获取data
function create() {
  ...
  for (i = j; i < j + 5; i++) {
    ...
    geta[m].append(p);
    const getdiv1 = document.getElementsByClassName('d-flex');
    h5.className = "mb-1";
    h5.innerHTML = data[i].name;
    let price = data[i].price / 100;//转换数据
    price = price.toFixed(2);
    small.innerHTML = price + "元";
    getdiv1[m].append(h5);
    getdiv1[m].append(small);
  }
}
create();
//翻页部分
...
function paginationfun() {
  pagination.innerHTML = "共 4 页，当前 " + page + " 页";//当前几页
  if (page === 1) {
...
pagination.innerHTML = "共 4 页，当前 " + page + " 页";
// 点击上一页
